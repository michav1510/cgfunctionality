...
class K2d_tree
{
  ...
  private:
	Node *m_root;					  // pointer to the m_root of the tree
	std::vector<Point2d> m_sort_by_x; // the points of the tree sorted by x
	std::vector<Point2d> m_sort_by_y; // the points of the tree sorted by y
	unsigned int m_size;			  // the number of the leaves for which we constructed the tree
	 ...
	 *  @returns a node pointer to the m_root of the tree
  ...
  public:
	...
	K2d_tree(std::vector<Point2d> points)
	{
		...
		m_size = m_sort_by_x.size();

		bool two_same = false;
		int siz = m_sort_by_x.size();
		for (int i = 0; i < siz - 1; i++)
		{
			if (m_sort_by_x[i].GetX() == m_sort_by_x[i + 1].GetX())
			{
				two_same = true;
			}
			if (m_sort_by_y[i].GetY() == m_sort_by_y[i + 1].GetY())
			{
				two_same = true;
			}
		}
		...
		{
			m_root = BuildTree(1, m_sort_by_x, m_sort_by_y);
		}
	}
	...
	~K2d_tree()
	{
		...
		std::vector<Node *> nodes = getAllNodes(m_root);
	...
	/**
	 * @returns a tree_iterator which points to the m_root of the tree
	 */
	tree_iterator begin() const
	{
		assert(m_root != 0);
		return tree_iterator(m_root);
	}
	...
	void addPoint(const Point2d poi)
	{
		...
		//second step : we have to add to the m_sort_by_x and m_sort_by_y the new point
		std::vector<Point2d>::iterator it = m_sort_by_x.begin();
		while (it != m_sort_by_x.end())
		{
			if (poi.GetX() <= (*it).GetX())
			{
				m_sort_by_x.insert(it, poi);
				break;
			}
			it++;
		}
		it = m_sort_by_y.begin();
		while (it != m_sort_by_y.end())
		{
			if (poi.GetY() <= (*it).GetY())
			{
				m_sort_by_y.insert(it, poi);
				break;
			}
			it++;
		}
